...
//!* `internal` contains the building blocks used to make new parsers. This is advanced usage and is far more involved than using the preexisting parsers, but is sometimes unavoidable.
//!
...
//!`SimpleResult<'a, I, T>` is a convenience type alias over Result<'a, I, T, Error<u8>>, and `Error<I>` is just a convenient "default" error type that will satisfy most users. For more sophisticated usage, one can always write a custom error type.
//!
...
//!        T: FromIterator<U> { ... }
//!```
//!This combinator construcs an iterator that will match elements `num` times, and collect them into any datatype you want.
